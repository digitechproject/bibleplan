...
export default function Navigation() {
  ...
  const { theme, toggleTheme, isMounted, user, profile, signOut } = useReadingPlan();
  ...
  // Si l'utilisateur est admin, ajouter l'onglet Admin
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-8">
            <Link href="/" className="flex items-center gap-2 text-amber-600 dark:text-amber-500 font-bold text-lg tracking-tight">
              ...
              <span>Défi Bible 2026 - 2030</span>
            </Link>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 md:hidden border-t border-zinc-200 dark:border-zinc-800 bg-white/90 dark:bg-zinc-950/90 backdrop-blur-md flex items-center justify-around h-16 px-2 safe-bottom transition-colors duration-200">
        {navItems.filter(item => item.path !== '/search' && item.path !== '/admin' && item.path !== '/history').map(item => {
          const isActive = pathname === item.path;
          return (
            <Link
              key={item.path}
          ...
          onClick={() => {
            if (user) {
              ...
              const actions = isAdmin 
                ? confirm(`Connecté (${user.email}).\nCliquez sur OK pour Aller à l'administration, ou ANNULER pour vous déconnecter.`)
                : confirm(`Connecté en tant que ${user.email}. Voulez-vous vous déconnecter ?`);
